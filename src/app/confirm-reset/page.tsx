...
export default async function ConfirmPage({ searchParams }: { searchParams: { [key: string]: string } }) {
  const { token_hash, type, next } = searchParams;
  const supabase =  await createClient();
  const redirectTo = next || '/'; // Default redirect if 'next' is not provided
  ...
  if (token_hash && type) {
    ...
    if (!error) {
      // Redirect to the "next" URL (e.g., reset-password page)
      redirect(redirectTo);
    } else {
    ...
    // Missing parameters, redirect to a fallback page
    redirect('/');
  }

  return null; // This won't render anything since we're redirecting
}
